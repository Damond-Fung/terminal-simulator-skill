...
const TypeWriter = ({ text, speed = 30, enabled = true, cursor = true, cursorStyle = 'block', className = '', onComplete, onChar, skipOnClick = false, pauseOnHover = false, as: Component = 'span', children, }) => {
    ...
    const renderContent = () => {
        ...
        return (jsxs(Fragment, { children: [displayedText, cursor && isTyping && (jsx("span", { className: `inline-block align-middle ${cursorClass}`, style: {
...
const TerminalLine = ({ line, showCopy = true, isAnimating = false, className = '', onCopy, }) => {
    ...
    return (jsxs("div", { className: `group relative mb-2 ${className}`, children: [jsxs("div", { className: "flex items-start gap-2", children: [line.type === 'command' && (jsx("span", { className: "shrink-0 select-none text-[var(--terminal-prompt, #fbbf24)]", "aria-hidden": "true", children: '>' })), jsxs("pre", { className: `flex-1 whitespace-pre-wrap break-words font-mono text-sm leading-relaxed ${getTypeStyles()}`, children: [line.text, isAnimating && (jsx("span", { className: "inline-block h-4 w-1.5 animate-pulse motion-reduce:animate-none bg-[var(--terminal-cursor, #fbbf24)] align-middle ml-0.5", "aria-hidden": "true" }))] })] }), showCopy && !isAnimating && line.text.trim() && (jsx("button", { onClick: handleCopy, className: "absolute right-1 top-0 opacity-0 transition-opacity group-hover:opacity-100 focus:opacity-100 p-1 rounded hover:bg-white/10", title: copied ? 'Copied!' : 'Copy', "aria-label": `Copy ${line.type}`, children: copied ? (jsx("svg", { className: "w-4 h-4 text-green-400", fill: "none", viewBox: "0 0 24 24", stroke: "currentColor", children: jsx("path", { strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 2, d: "M5 13l4 4L19 7" }) })) : (jsx("svg", { className: "w-4 h-4 text-gray-400", fill: "none", viewBox: "0 0 24 24", stroke: "currentColor", children: jsx("path", { strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 2, d: "M8 16H6a2 2 0 01-2-2V6a2 2 0 012-2h8a2 2 0 012 2v2m-6 12h8a2 2 0 002-2v-8a2 2 0 00-2-2h-8a2 2 0 00-2 2v8a2 2 0 002 2z" }) })) }))] }));
};
...
const TerminalSimulator = ({ mode = 'free-type', steps = [], commands = [], title = 'Terminal', variant = 'macos', theme = 'dark', autoPlay = false, controls = true, welcomeMessage = 'Welcome! Type /help to get started.', prompt = '>', className = '', onStepComplete, onAllComplete, onCommand, }) => {
    ...
    const allCommands = useMemo(() => [...createDefaultCommands(), ...commands], [commands]);
    // 初始化终端模拟器
    useEffect(() => {
    ...
    const WindowChrome = () => {
        ...
        return (jsxs("div", { className: "flex items-center gap-2 px-4 py-2.5 bg-[var(--terminal-header-bg, #1c1917)]", children: [dots.map((color, i) => (jsx("span", { className: `h-3 w-3 rounded-full ${color}` }, i))), jsx("span", { className: "ml-2 text-xs text-[var(--terminal-header-text, #e7e5e4)]/60 truncate", children: title })] }));
    };
    // 控制按钮
    const Controls = () => {
        if (!controls)
        ...
        if (mode === 'guided') {
            // 引导模式控制
            if (state?.isComplete) {
                return (jsxs("div", { className: "flex items-center justify-between border-t border-[var(--terminal-border, #44403c)] bg-[var(--terminal-footer-bg, #171412)] px-4 py-3", children: [jsx("span", { className: "text-xs text-green-400", children: "All steps complete!" }), jsx("button", { onClick: handleReset, className: "min-h-[44px] rounded-lg bg-[var(--terminal-button-bg, #f59e0b)] px-5 py-2 text-sm font-semibold text-white hover:opacity-90", children: "Reset" })] }));
            }
            if (state?.isTyping) {
                return (jsxs("div", { className: "flex items-center justify-between border-t border-[var(--terminal-border, #44403c)] bg-[var(--terminal-footer-bg, #171412)] px-4 py-3", children: [jsx("span", { className: "text-xs text-[var(--terminal-text-muted, #e7e5e4)]/70", children: "Running..." }), jsx("button", { onClick: handleSkip, className: "min-h-[44px] rounded-lg bg-[var(--terminal-button-bg, #f59e0b)] px-5 py-2 text-sm font-semibold text-white hover:opacity-90", children: "Skip" })] }));
            }
            const currentStep = steps[state?.currentStep ?? 0];
            const isFirstStep = (state?.currentStep ?? -1) < 0;
            return (jsxs("div", { className: "flex items-center justify-between border-t border-[var(--terminal-border, #44403c)] bg-[var(--terminal-footer-bg, #171412)] px-4 py-3", children: [jsx("div", { className: "flex-1", children: currentStep && (jsx("p", { className: "text-xs text-[var(--terminal-text-muted, #e7e5e4)]/70", children: currentStep.description || `Step ${(state?.currentStep ?? 0) + 1}: ${currentStep.command}` })) }), jsx("button", { onClick: isFirstStep ? handleStart : handleNext, className: "min-h-[44px] rounded-lg bg-[var(--terminal-button-bg, #f59e0b)] px-5 py-2 text-sm font-semibold text-white hover:opacity-90", children: isFirstStep ? 'Run' : currentStep?.nextLabel || 'Next Step' })] }));
        }
        // 自由输入模式不需要额外控制
        return null;
    };
    if (!state)
        return null;
    return (jsxs("div", { className: `overflow-hidden rounded-xl border border-[var(--terminal-border, #44403c)] shadow-2xl ${className}`, role: "application", "aria-label": title, "data-terminal-theme": theme, style: currentThemeStyle, children: [jsx(WindowChrome, {}), jsxs("div", { ref: scrollRef, className: "overflow-y-auto bg-[var(--terminal-bg, #1c1917)] p-4 font-mono text-sm min-h-[280px] max-h-[480px]", dir: "ltr", children: [state.lines.map((line) => (jsx(TerminalLine, { line: line, showCopy: true }, line.id))), state.pendingLine && (jsx(TerminalLine, { line: {
                            id: 'pending-line',
                            type: state.pendingLine.type,
                            text: state.pendingLine.text,
                            timestamp: Date.now(),
                        }, showCopy: false, isAnimating: true })), mode === 'free-type' && (jsxs("div", { className: "flex items-center gap-2 mt-2", children: [jsx("span", { className: "shrink-0 select-none text-[var(--terminal-prompt, #fbbf24)]", "aria-hidden": "true", children: prompt }), jsx("input", { ref: inputRef, type: "text", value: input, onChange: handleInputChange, onKeyDown: handleKeyDown, className: "flex-1 bg-transparent text-[var(--terminal-fg, #e7e5e4)] outline-none placeholder:text-[var(--terminal-fg, #e7e5e4)]/30 caret-[var(--terminal-cursor, #fbbf24)]", placeholder: "Type a command...", "aria-label": "Terminal command input", autoComplete: "off", spellCheck: false })] })), state.suggestions.length > 1 && (jsx("div", { className: "mt-1 flex flex-wrap gap-2 pl-5", children: state.suggestions.map((suggestion, index) => (jsx("span", { className: `text-xs ${index === state.suggestionIndex ? 'text-[var(--terminal-prompt, #fbbf24)]' : 'text-[var(--terminal-fg, #e7e5e4)]/50'}`, children: suggestion }, suggestion))) }))] }), jsx(Controls, {}), jsx("div", { className: "sr-only", "aria-live": "polite", "aria-atomic": "false", children: state.lines.length > 0 && state.lines[state.lines.length - 1].type === 'output'
...
export { TerminalLine, TerminalSimulator, TypeWriter, VERSION, createDefaultCommands, createSmartTypeWriter, prefersReducedMotion };
